...
async def createUrl(long_url: HttpURL):
    short_url = await run_in_threadpool(insert_value, long_url.url)
    return {'short_url': short_url}


@app.get('/{short_url}')
async def urlFetch(short_url: str = Path(
        description='Short URL path',
        pattern='^[a-zA-Z0-9]{5}$'
        ), info: bool = False
    ):
    long_url, c_date = await run_in_threadpool(get_long_url, short_url)
    if info:
